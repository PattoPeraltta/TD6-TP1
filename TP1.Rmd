...
y_true_num <- ifelse(testeo$y == "yes", 1, 0)
y_pred_num <- ifelse(pred_class == "yes", 1, 0)

# Matriz de confusión y métricas
matriz_conf_tbl <- MLmetrics::ConfusionMatrix(y_pred = y_pred_num, y_true = y_true_num)

accuracy  <- MLmetrics::Accuracy(y_pred = y_pred_num, y_true = y_true_num)
precision <- MLmetrics::Precision(y_true = y_true_num, y_pred = y_pred_num, positive = "1")
recall    <- MLmetrics::Recall(y_true = y_true_num,    y_pred = y_pred_num, positive = "1")
f1        <- MLmetrics::F1_Score(y_true = y_true_num,  y_pred = y_pred_num, positive = "1")

# --- Gráficos: Matriz de confusión (conteos y proporciones) ------------------

# Conteos (CAMBIA la fuente de la tabla y se mapean 0/1 -> no/yes)
cm_tbl <- as.table(matriz_conf_tbl)          # <— antes venía de matriz_conf$table (caret)
cm_df  <- as.data.frame(cm_tbl)
names(cm_df) <- c("real", "pred", "freq")

# mapear etiquetas para que el gráfico muestre "no"/"yes"
map01 <- c("0" = "no", "1" = "yes")
cm_df$real <- factor(map01[as.character(cm_df$real)], levels = c("no", "yes"))
cm_df$pred <- factor(map01[as.character(cm_df$pred)], levels = c("no", "yes"))
...
# Proporciones por clase real (SIN CAMBIOS en la lógica)
...
# ROC y AUC
# (Dejamos pROC para la curva, pero el AUC lo calculamos con MLmetrics SOBRE pred_prob + y_true_num)
roc_curve <- pROC::roc(
  response = testeo$y,
  predictor = pred_prob,
  levels = c("no", "yes"),
...
auc_value <- MLmetrics::AUC(y_pred = as.numeric(pred_prob), y_true = y_true_num)  # <— CAMBIA

# Tabla resumen (usa las métricas calculadas con MLmetrics y este AUC de MLmetrics)
resumen <- data.frame(
...
knitr::kable(resumen, digits = 4, caption = "Punto 4 — Métricas (Test) con MLmetrics")


# Gráfico de la curva ROC usando ggplot2 (SIN CAMBIOS, solo cambia el caption porque usa el AUC anterior)
...
reference_df <- data.frame(x = c(0, 1), y = c(0, 1))
...
ggplot() +
  ...
  labs(
    ...
    y = "Sensibilidad (TPR)",
    caption = paste("AUC (MLmetrics) =", round(auc_value, 4))  # <— CAMBIA el texto del caption
  ) +
  theme_minimal() +
  theme(
...
# Convertir a 0/1
y_true_num_opt <- ifelse(testeo$y == "yes", 1, 0)
y_pred_num_opt <- ifelse(pred_class_opt == "yes", 1, 0)

# Calcular métricas con MLmetrics
accuracy_opt  <- MLmetrics::Accuracy(y_pred = y_pred_num_opt, y_true = y_true_num_opt)
precision_opt <- MLmetrics::Precision(y_true = y_true_num_opt, y_pred = y_pred_num_opt, positive = "1")
recall_opt    <- MLmetrics::Recall(y_true = y_true_num_opt,    y_pred = y_pred_num_opt, positive = "1")
f1_opt        <- MLmetrics::F1_Score(y_true = y_true_num_opt,  y_pred = y_pred_num_opt, positive = "1")

# Tabla resumen
metricas_optimizado <- data.frame(
  Metrica = c("Accuracy", "Precision", "Recall", "F1"),
  Valor = c(accuracy_opt, precision_opt, recall_opt, f1_opt),
  stringsAsFactors = FALSE
)

knitr::kable(metricas_optimizado,
  digits = 4,
  caption = "Métricas del árbol optimizado en testeo (MLmetrics)"
)
```
...
prob_basico <- predict(arbol_basico, newdata = testeo, type = "prob")
col_pos_b   <- if ("yes" %in% colnames(prob_basico)) "yes" else tail(colnames(prob_basico), 1)

prob_opt    <- predict(mejor_arbol, newdata = testeo, type = "prob")
col_pos_o   <- if ("yes" %in% colnames(prob_opt)) "yes" else tail(colnames(prob_opt), 1)

# ROC 
roc_basico <- pROC::roc(testeo$y, prob_basico[, col_pos_b], levels = c("no", "yes"), direction = "<")
roc_optimizado <- pROC::roc(testeo$y, prob_opt[, col_pos_o], levels = c("no", "yes"), direction = "<")

# Extraer coordenadas una sola vez
cb <- pROC::coords(roc_basico, ret = "all")
co <- pROC::coords(roc_optimizado, ret = "all")


roc_data <- data.frame(
  FPR = c(1 - cb$specificity, 1 - co$specificity),
  TPR = c(cb$sensitivity,     co$sensitivity),
  Modelo = c(rep("Árbol básico", length(cb$specificity)),
             rep("Árbol optimizado", length(co$specificity)))
)

# Gráfico comparativo
ggplot(roc_data, aes(x = FPR, y = TPR, color = Modelo)) +
  geom_line(linewidth = 1) +   # <- grosor consistente
  geom_abline(slope = 1, intercept = 0, 
              linetype = "dashed", color = "gray", linewidth = 0.8) +  # <- línea de referencia
  labs(
  ...
  scale_color_manual(values = c("Árbol básico" = "red", 
                                "Árbol optimizado" = "blue")) +
  theme_minimal() +
  theme(
    plot.title   = element_text(hjust = 0.5),
    plot.caption = element_text(hjust = 0.5, face = "bold")
...
# --- Baseline (árbol optimizado original) en testeo con MLmetrics::AUC ----------
baseline_test_auc <- evaluar_auc(mejor_arbol, testeo)

# --- Optimización para cada versión corrupta (mismo enfoque que Sección 4) -------
resultados_versiones <- list()
mejores_modelos_versiones <- list()

for (nm in names(versiones_corruptas)) {
  # tabla de resultados para esta versión
  ...
  # mejor combinación para esta versión
...
# --- Evaluación comparativa en testeo (AUC con MLmetrics) -----------------------
# Asegurar orden V1..V6 en caso de que la lista no venga ordenada
orden_versions <- paste0("V", seq_along(porcentajes_corrupcion))

resultados_test_comparativo <- data.frame(
  Version = c("Baseline (0%)",
              paste0(orden_versions, " (", porcentajes_corrupcion * 100, "%)")),
  AUC_Test = c(
    baseline_test_auc,
    vapply(orden_versions, function(nm)
      evaluar_auc(mejores_modelos_versiones[[nm]], testeo), numeric(1))
  ),
  Porcentaje_Corrupcion = c(0, porcentajes_corrupcion * 100),
  stringsAsFactors = FALSE
)

# --- Agregar métricas adicionales (Accuracy/Precision/Recall/F1 con MLmetrics) ---
for (i in 1:nrow(resultados_test_comparativo)) {

  # elegir el modelo correspondiente a la fila
  if (i == 1) {
    modelo_actual <- mejor_arbol  # baseline
  } else {
    modelo_actual <- mejores_modelos_versiones[[orden_versions[i - 1]]]
  }

  # clases predichas en testeo
  pred_class <- predict(modelo_actual, newdata = testeo, type = "class")
  pred_class <- factor(pred_class, levels = levels(testeo$y))

  # convertir a 0/1(1 = "yes")
  y_true_num <- ifelse(testeo$y  == "yes", 1, 0)
  y_pred_num <- ifelse(pred_class == "yes", 1, 0)

  # métricas MLmetrics
  resultados_test_comparativo$Accuracy[i]  <- MLmetrics::Accuracy(y_pred = y_pred_num, y_true = y_true_num)
  resultados_test_comparativo$Precision[i] <- MLmetrics::Precision(y_true = y_true_num, y_pred = y_pred_num, positive = "1")
  resultados_test_comparativo$Recall[i]    <- MLmetrics::Recall(y_true = y_true_num,    y_pred = y_pred_num, positive = "1")
  resultados_test_comparativo$F1[i]        <- MLmetrics::F1_Score(y_true = y_true_num,  y_pred = y_pred_num, positive = "1")
}

# --- Mostrar tabla comparativa ---------------------------------------------------
knitr::kable(resultados_test_comparativo,
  digits = 4,
  caption = "Rendimiento comparativo (Test): Baseline vs Versiones con etiquetas corruptas"
)
```
